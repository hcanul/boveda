...
namespace App\Http\Livewire\Morelos;

use App\Models\Coin;
use App\Models\Moneymorelos;
use Illuminate\Support\Facades\DB;
use Livewire\Component;
use Livewire\WithPagination;

class MorelosController extends Component
{
    use WithPagination;
    ...
    public function mount()
    {
        ...
        $this->componentName = 'BOVEDA MORELOS';
        $this->selected_id = 0;
        $this->show = false;
        $this->total = null;
        $this->operacion = null;
    ...
    {
        if($this->search){
            $data = Moneymorelos::where('created_at', 'like', '%' . $this->search .'%')->paginate($this->pagination);
            $this->superTotal = $this->supersuma($this->search);
        }else{
            $data = Moneymorelos::orderBy('id', 'asc')->paginate($this->pagination);
            $this->superTotal = $this->supersuma(null);
        }
        return view('livewire.morelos.morelos', ['data' => $data, 'denominacion' => Coin::all()])
    ...
    public function Editar(Moneymorelos $moneymorelos)
    {
        $this->Billete1000 = $moneymorelos->bmil;
        $this->Billete500 = $moneymorelos->bqui;
        $this->Billete200 = $moneymorelos->bdoc;
        $this->Billete100 = $moneymorelos->bcie;
        $this->Billete50 = $moneymorelos->bcin;
        $this->Billete20 = $moneymorelos->bvei;
        $this->Moneda20 = $moneymorelos->mvei;
        $this->Moneda10 = $moneymorelos->mdie;
        $this->Moneda5 = $moneymorelos->mcin;
        $this->Moneda2 = $moneymorelos->mdos;
        $this->Moneda1 = $moneymorelos->muno;
        $this->Moneda50c = $moneymorelos->mpci;
    ...
    public function Destroy(Moneymorelos $moneymorelos)
    {
        $moneymorelos->delete();

...
class ManagerController extends Component
{
    ...
    public function mount()
    {
        ...
        $this->componentName = 'CÁLCULO GERENTE';
    }

    public function render()
    {
        if($this->search){
            $data = Manager::where('created_at', 'like', '%' . $this->search .'%')->paginate($this->pagination);
        }else{
            $data = Manager::orderBy('id', 'asc')->paginate($this->pagination);
        }

        if ($this->city_id)
        {
            $asesores = Employes::whereCityId($this->city_id)->whereTypeemployesId(4)->get();
        }
        else
        {
    ...
    public function Editar($id)
    {
        ...
        $this->porsrinicio = $manage->porsrinicio;
        $this->clientesf = $manage->clientesf;
        $this->carterafinal = $manage->carterafinal;
        $this->srfinal = $manage->srfinal;
        $this->porsrfinal = $manage->porsrfinal;
        $this->category_adviser_id = $manage->category_adviser_id;
        $this->metacoloca = $manage->metacoloca;
        $this->colocadoreal = $manage->colocadoreal;
        $this->poralcancemetacoloca = $manage->poralcancemetacoloca;
        $this->diferenciaclientes = $manage->diferenciaclientes;
        $this->diferenciacartera = $manage->diferenciacartera;
        $this->bonoclientes = $manage->bonoclientes;
        $this->bonoccolocacion = $manage->bonoccolocacion;
        $this->bonoexcelencia = $manage->bonoexcelencia;
        $this->bonofina = $manage->bonofina;
        $this->base = $manage->base;
        $this->rmetac = $manage->rmetac;
        $this->redsr = $manage->redsr;
    ...
    public function Update()
    {
        ...
        $manager = Manager::find($this->selected_id);

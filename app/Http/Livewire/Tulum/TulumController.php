...
namespace App\Http\Livewire\Tulum;

use App\Models\Coin;
use App\Models\Moneytulums;
use Illuminate\Support\Facades\DB;
use Livewire\Component;
use Livewire\WithPagination;

class TulumController extends Component
{
    use WithPagination;
    ...
    public function mount()
    {
        ...
        $this->componentName = 'BOVEDA TULUM';
        $this->selected_id = 0;
        $this->show = false;
        $this->total = null;
        $this->operacion = null;
    ...
    {
        if($this->search){
            $data = Moneytulums::where('created_at', 'like', '%' . $this->search .'%')->paginate($this->pagination);
            $this->superTotal = $this->supersuma($this->search);
        }else{
            $data = Moneytulums::orderBy('id', 'asc')->paginate($this->pagination);
            $this->superTotal = $this->supersuma(null);
        }
        return view('livewire.tulum.tulum', ['data' => $data, 'denominacion' => Coin::all()])
    ...
    public function Editar(Moneytulums $moneytulums)
    {
        $this->Billete1000 = $moneytulums->bmil;
        $this->Billete500 = $moneytulums->bqui;
        $this->Billete200 = $moneytulums->bdoc;
        $this->Billete100 = $moneytulums->bcie;
        $this->Billete50 = $moneytulums->bcin;
        $this->Billete20 = $moneytulums->bvei;
        $this->Moneda20 = $moneytulums->mvei;
        $this->Moneda10 = $moneytulums->mdie;
        $this->Moneda5 = $moneytulums->mcin;
        $this->Moneda2 = $moneytulums->mdos;
        $this->Moneda1 = $moneytulums->muno;
        $this->Moneda50c = $moneytulums->mpci;
    ...
    public function Destroy(Moneytulums $moneytulums)
    {
        $moneytulums->delete();

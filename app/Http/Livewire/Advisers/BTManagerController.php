...
class BTManagerController extends Component
{
    use WithPagination;
    ...
    {
        if($this->search){
            $data = branch_type_manager::where('name', 'like', '%' . $this->search .'%')->paginate($this->pagination);
        }else{
            $data = branch_type_manager::orderBy('id', 'asc')->paginate($this->pagination);
        }

        return view('livewire.advisers.manager.component', ['data' => $data])
    ...
    public function Store()
    {
        ...
        branch_type_manager::create(['name' => $this->name, 'rangeinit'=>$this->rangeinit, 'rangefin' => $this->rangefin]);
    ...
    {
        $branch = branch_type_manager::find($id);
    ...
    public function Update()
    {
        ...
        $branch = branch_type_manager::find($this->selected_id);
    ...
    public function Destroy(branch_type_manager $branch)
    {

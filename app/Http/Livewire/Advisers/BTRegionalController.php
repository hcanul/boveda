...
class BTRegionalController extends Component
{
    use WithPagination;
    ...
    public function mount()
    {
        ...
        $this->componentName = 'SECCIÓN DE TIPOS GERENTE';
    }

    public function render()
    {
        if($this->search){
            $data = branch_type_regional::where('name', 'like', '%' . $this->search .'%')->paginate($this->pagination);
        }else{
            $data = branch_type_regional::orderBy('id', 'asc')->paginate($this->pagination);
        }

        return view('livewire.advisers.regional.component', ['data' => $data])
    ...
    public function resetUI()
    {
        ...
        $this->selected_id = 0;
        $this->pageTitle = 'Listado';
        $this->componentName = 'SECCIÓN DE TIPOS COORDINADOR';
    ...
    public function Store()
    {
        ...
        branch_type_regional::create(['name' => $this->name, 'rangeinit'=>$this->rangeinit, 'rangefin' => $this->rangefin]);
    ...
    {
        $branch = branch_type_regional::find($id);
    ...
    public function Update()
    {
        ...
        $branch = branch_type_regional::find($this->selected_id);
    ...
    public function Destroy(branch_type_regional $branch)
    {

...
namespace App\Http\Livewire\Playa2;

use App\Models\Coin;
use App\Models\Moneyplaya2s;
use Illuminate\Support\Facades\DB;
use Livewire\Component;
use Livewire\WithPagination;

class Playa2Controller extends Component
{
    use WithPagination;
    ...
    public function mount()
    {
        ...
        $this->componentName = 'BOVEDA PLAYA2';
        $this->selected_id = 0;
        $this->show = false;
        $this->total = null;
        $this->operacion = null;
    ...
    {
        if($this->search){
            $data = Moneyplaya2s::where('created_at', 'like', '%' . $this->search .'%')->paginate($this->pagination);
            $this->superTotal = $this->supersuma($this->search);
        }else{
            $data = Moneyplaya2s::orderBy('id', 'asc')->paginate($this->pagination);
            $this->superTotal = $this->supersuma(null);
        }
        return view('livewire.playa2.playa2', ['data' => $data, 'denominacion' => Coin::all()])
    ...
    public function Editar(Moneyplaya2s $moneyplaya2s)
    {
        $this->Billete1000 = $moneyplaya2s->bmil;
        $this->Billete500 = $moneyplaya2s->bqui;
        $this->Billete200 = $moneyplaya2s->bdoc;
        $this->Billete100 = $moneyplaya2s->bcie;
        $this->Billete50 = $moneyplaya2s->bcin;
        $this->Billete20 = $moneyplaya2s->bvei;
        $this->Moneda20 = $moneyplaya2s->mvei;
        $this->Moneda10 = $moneyplaya2s->mdie;
        $this->Moneda5 = $moneyplaya2s->mcin;
        $this->Moneda2 = $moneyplaya2s->mdos;
        $this->Moneda1 = $moneyplaya2s->muno;
        $this->Moneda50c = $moneyplaya2s->mpci;
    ...
    public function Destroy(Moneyplaya2s $moneyplaya2s)
    {
        $moneyplaya2s->delete();

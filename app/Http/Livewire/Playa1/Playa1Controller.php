...
namespace App\Http\Livewire\Playa1;

use App\Models\Coin;
use App\Models\Moneyplaya1s;
use Illuminate\Support\Facades\DB;
use Livewire\Component;
use Livewire\WithPagination;

class Playa1Controller extends Component
{
    use WithPagination;
    ...
    public function mount()
    {
        ...
        $this->componentName = 'BOVEDA PLAYA1';
        $this->selected_id = 0;
        $this->show = false;
        $this->total = null;
        $this->operacion = null;
    ...
    {
        if($this->search){
            $data = Moneyplaya1s::where('created_at', 'like', '%' . $this->search .'%')->paginate($this->pagination);
            $this->superTotal = $this->supersuma($this->search);
        }else{
            $data = Moneyplaya1s::orderBy('id', 'asc')->paginate($this->pagination);
            $this->superTotal = $this->supersuma(null);
        }
        return view('livewire.playa1.playa1', ['data' => $data, 'denominacion' => Coin::all()])
    ...
    public function Editar(Moneyplaya1s $moneyplaya1s)
    {
        $this->Billete1000 = $moneyplaya1s->bmil;
        $this->Billete500 = $moneyplaya1s->bqui;
        $this->Billete200 = $moneyplaya1s->bdoc;
        $this->Billete100 = $moneyplaya1s->bcie;
        $this->Billete50 = $moneyplaya1s->bcin;
        $this->Billete20 = $moneyplaya1s->bvei;
        $this->Moneda20 = $moneyplaya1s->mvei;
        $this->Moneda10 = $moneyplaya1s->mdie;
        $this->Moneda5 = $moneyplaya1s->mcin;
        $this->Moneda2 = $moneyplaya1s->mdos;
        $this->Moneda1 = $moneyplaya1s->muno;
        $this->Moneda50c = $moneyplaya1s->mpci;
    ...
    public function Destroy(Moneyplaya1s $moneyplaya1s)
    {
        $moneyplaya1s->delete();
